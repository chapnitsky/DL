...
def momentum(model, X_train, y_train, minibatch_size):
    ...
    for iter in range(1, n_iter + 1):
        ...
        grad = get_minibatch_grad(model, X_mini, y_mini)
        for layer in grad:
            velocity[layer] = mu * velocity[layer] - 0.1 * grad[layer]
...
def nesterov(model, X_train, y_train, minibatch_size):
    ...
    for iter in range(1, n_iter + 1):
        ...
        for layer in grad:
            velocity[layer] = gamma * velocity[layer] - 0.1 * grad[layer]
            model[layer] += velocity[layer]
    return model
...
def adagrad(model, X_train, y_train, minibatch_size):
    ...
    for iter in range(1, n_iter + 1):
        ...
        for k in grad:
            cache[k] += grad[k] ** 2
            model[k] += -0.1 * grad[k] / (np.sqrt(cache[k]))
    return model
...
def adam(model, X_train, y_train, minibatch_size):
    ...
    for iter in range(1, n_iter + 1):
        ...
        for k in grad:
            ...
            model[k] += -0.1 * m_k_hat / np.sqrt(r_k_hat)
    return model
